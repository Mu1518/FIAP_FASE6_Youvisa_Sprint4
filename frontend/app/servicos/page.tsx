        ...
        <main className="flex-grow py-10 md:py-20 px-4 md:px-6">
          ...
          <section className="max-w-4xl mx-auto text-center mb-12 md:mb-20">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-6xl font-black tracking-tight text-slate-800 mb-6 leading-tight">
              {t('Soluções completas para')}<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600">
                {t('cada tipo de visto')}
              </span>
            </h1>
            <p className="text-lg text-slate-600 max-w-2xl mx-auto leading-relaxed">
              {t('Combinamos expertise humana com inteligência artificial para tornar cada processo mais ágil, seguro e transparente. Escolha o serviço que melhor atende à sua necessidade.')}
            </p>
          </section>
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row md:items-start gap-6 mb-10">
                    ...
                    <div className="flex-grow">
                      <h2 className="text-3xl font-black text-slate-800 mb-1">{t(s.titulo)}</h2>
                      <p className={`text-sm font-semibold mb-3 ${c.text}`}>{t(s.subtitulo)}</p>
                      <p className="text-slate-600 leading-relaxed max-w-3xl">{t(s.descricao)}</p>
                    </div>
                    <div className="flex md:flex-col gap-4 md:gap-3 shrink-0 text-sm">
                      <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${c.badge} ${c.border}`}>
                        ...
                        <div>
                          <div className="text-xs opacity-70">{t('Prazo médio')}</div>
                          <div className="font-semibold">{t(s.prazo)}</div>
                        </div>
                      </div>
                      <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${c.badge} ${c.border}`}>
                        <span className={`material-symbols-outlined text-[16px] ${c.text}`}>event_available</span>
                        <div>
                          <div className="text-xs opacity-70">{t('Validade')}</div>
                          <div className="font-semibold">{t(s.validade)}</div>
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <ol className="space-y-3">
                        {s.etapas.map((etapa, i) => (
                          <li key={i} className="flex items-start gap-3">
                            ...
                            <span className="text-sm text-slate-600 pt-0.5">{t(etapa)}</span>
                          </li>
                        ))}
                      </ol>
                    </div>
                  ...
                  <div className="mt-10 pt-8 border-t border-black/5 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p className="text-sm text-slate-500">
                      {t('Tire suas dúvidas com nosso assistente IA ou inicie sua solicitação agora.')}
                    </p>
                    <Link
                      href="/clientes"
          ...
          <section className="max-w-3xl mx-auto mt-24 text-center">
            <div className="bg-gradient-to-br from-blue-600 to-purple-600 rounded-3xl p-12 text-white shadow-2xl shadow-blue-600/20">
              <h2 className="text-3xl font-black mb-4">{t('Ainda com dúvidas?')}</h2>
              <p className="text-white/80 text-lg mb-8 leading-relaxed">
                {t('Nosso assistente de IA está disponível 24 horas por dia para responder suas perguntas sobre vistos, prazos e documentação necessária.')}
              </p>
