...
export default function AdminLoginForm() {
  ...
  const handleVerificarOTP = async (e: FormEvent) => {
    ...
    try {
      ...
      salvarAuth(res.token, { ...res.funcionario, tipo: 'funcionario' });
      ...
      <form className="space-y-5" onSubmit={handleVerificarOTP}>
        ...
        <div className="pt-4 flex flex-col gap-3">
          <button
            ...
            className="w-full flex items-center justify-center h-12 rounded-xl bg-slate-800 text-white font-bold shadow-lg shadow-slate-800/30 hover:bg-slate-900 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
    ...
    <form className="space-y-5" onSubmit={handleEnviarOTP}>
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white/50 focus:bg-white focus:ring-2 focus:ring-slate-600/20 focus:border-slate-600 outline-none transition-all"
